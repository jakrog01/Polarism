...
def test_solver_run_matrix(tmp_path, solver: str, grid_type: str, reservoir: str) -> None:
    ...
    if solver in {"rk4-cuda", "rk4-cuda-v100", "ifrk4-fft-cuda"} and not hasattr(compute_engine.xp, "cuda"):
        pytest.skip("CUDA solver requires an available CUDA device")
    cfg = small_config(grid__nx=32, grid__ny=32, grid__lx=40.0, grid__ly=40.0, grid__grid_type=grid_type, solver__method=solver, solver__dt=0.01, solver__total_time=0.5, reservoir__reservoir_type=reservoir, result__save_results=True, result__save_hdf5=True, result__save_interval=7, result__batch_size=10, result__output_directory=str(tmp_path), compute_engine=ComputeEngineParameters(use_gpu=False))
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", category=UserWarning)
        controller = SimulationController(cfg)
        controller.run()
    assert np.isfinite(np.asarray(controller.state.psi)).all()
    assert np.isfinite(np.asarray(controller.reservoir.get_reservoir_density())).all()
