"""Physics simulation kernel for the dot-response-fit pipeline.

Handles the quadratic-double reservoir by reading ``(nR, nI)`` state.
"""
...
_FIELD_SPECS: dict[str, np.dtype] = {
    ...
    "nR": np.dtype(np.float64),
    "nI": np.dtype(np.float64),
    "Pump": np.dtype(np.float64),
}


def _check_condensation_kspace(psi: object, xp: object) -> float:
    """Return the k=0 power fraction of *psi*."""
...
def _compute_pump_fast(
    ...
    """Build the total pump field and per-laser peak values."""
    P_total = P_zero.copy()
    per_laser_max = []
...
def _get_reservoir_fields(reservoir: object) -> tuple:
    """Return ``(nR, nI)`` from either reservoir type."""
    if hasattr(reservoir, "get_reservoir_densities"):
        return reservoir.get_reservoir_densities()
...
def run_simulation_from_config(
    ...
    """Run a simulation and write output to HDF5 and a scalar sidecar.
    ...
    output_policy : OutputPolicy or None
        Recording cadence and archival flags.  Defaults used when ``None``.
    ...
    scalar_names = ["psi_sq_max", "nR_max", "nI_max", "k0_frac", "P_max"]
    ...
    spatial_maxes = [float(xp.max(p)) for p in spatial_profiles]
    ...
    try:
        for step in trange(n_steps, desc=f"  {routine_name}"):
            ...
            if record_scalars or record_fields:
                nR_field, nI_field = _get_reservoir_fields(reservoir)
                psi_sq = xp.abs(state.psi) ** 2
                k0_f = (
                    _check_condensation_kspace(state.psi, xp)
                    if step % CHECK_EVERY == 0
                    else 0.0
                )
                scalars = {
                    "psi_sq_max": float(xp.max(psi_sq)),
                    "nR_max": float(xp.max(nR_field)),
                    "nI_max": float(xp.max(nI_field)),
                    "k0_frac": k0_f,
            ...
            if record_fields:
                ...
                writer.record(
                    (step + 1) * dt,
                    {"psi": state.psi, "nR": nR_field, "nI": nI_field, "Pump": P_total},
                    scalars,
                )
