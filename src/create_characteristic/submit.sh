...
echo "========================================"
echo " Create-Characteristic  |  2D sweep"
...
export PYTHONPATH="${PROJECT_ROOT}:${CC_DIR}:${PYTHONPATH:-}"
...
python3 -m create_characteristic.config.validator --config "$CONFIG" --slurm-env "$SLURM_ENV" || {
    echo "Aborting: fix the validation errors above." >&2; exit 1
}
echo ""

mapfile -t _PARSED < <(python3 - "$CONFIG" "$PROJECT_ROOT" "$CC_DIR" <<'PYEOF'
import sys
sys.path.insert(0, sys.argv[2])
sys.path.insert(0, sys.argv[3])
from create_characteristic.config.loader import load_config, get_sweep_config, generate_grid_points
cfg = load_config(sys.argv[1])
sweep = get_sweep_config(cfg)
points = generate_grid_points(cfg)
print(len(points))
print(int(sweep.get("max_concurrent", 16)))
PYEOF
)
N_POINTS="${_PARSED[0]}"
MAX_CONCURRENT="${_PARSED[1]}"
if [[ "$N_POINTS" -lt 1 ]]; then
    echo "ERROR: no grid points to submit." >&2
    exit 1
fi
POINT_LAST_INDEX=$((N_POINTS - 1))
if [[ "$N_POINTS" -gt 1 ]]; then
    ARRAY_ENABLED=1
    ARRAY_SPEC="0-$((N_POINTS - 2))%${MAX_CONCURRENT}"
else
    ARRAY_ENABLED=0
    ARRAY_SPEC=""
fi
...
GPU_TIME="${CHARACTERISTIC_TIME:-${SWEEP_TIME:-${SLURM_TIME}}}"
...
echo "  Points    : $N_POINTS  (array $ARRAY_SPEC)"
echo "  GPU time  : $GPU_TIME  (set CHARACTERISTIC_TIME in slurm.env to override)"
...
run_dir = os.path.join(base, f"cc_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{chash}")
os.makedirs(run_dir, exist_ok=False)
print(run_dir)
PYEOF
)
else
    RUN_DIR="${RUNS_BASE_DIR}/cc_DRY_$(date +%Y%m%d_%H%M%S)"
...
if [[ $DRY_RUN -eq 0 ]]; then
    ...
    python3 - "$RUN_DIR" "$CONFIG" "$PROJECT_ROOT" "$CC_DIR" <<'PYEOF'
import sys, json
sys.path.insert(0, sys.argv[3])
sys.path.insert(0, sys.argv[4])
from create_characteristic.config.loader import load_config, generate_grid_points
run_dir, cfg_path = sys.argv[1], sys.argv[2]
cfg = load_config(cfg_path)
points = generate_grid_points(cfg)
with open(f"{run_dir}/point_index.json", "w") as f:
    json.dump(points, f, indent=2)
    f.write("\n")
print(f"  point_index.json: {len(points)} points")
PYEOF
...
sys.path.insert(0, sys.argv[3])
sys.path.insert(0, sys.argv[4])
from create_characteristic.manifest.io import init_manifest
run_dir, cfg_path = sys.argv[1], sys.argv[2]
with open(f"{run_dir}/point_index.json") as f:
    n = len(json.load(f))
init_manifest(run_dir, cfg_path, n)
print("  Manifest initialised.")
PYEOF

    mkdir -p "$RUN_DIR/points"
    echo ""
fi
...
if [[ $ARRAY_ENABLED -eq 0 ]]; then
    SWEEP_ARRAY_JOB=""
elif [[ $DRY_RUN -eq 1 ]]; then
    echo "[1a] sweep array  (array=${ARRAY_SPEC}, time=${GPU_TIME})  [DRY RUN]"
    ...
    SWEEP_ARRAY_JOB=$(_rysy_sbatch \
        ...
        "$CLUSTER_DIR/job_gpu.sh" \
            python -m create_characteristic.stages.gpu.run_point \
                --run-dir "$RUN_DIR")
    echo "[1a] sweep array  -> Rysy job $SWEEP_ARRAY_JOB  (array=${ARRAY_SPEC}, time=${GPU_TIME})"
...
if [[ $DRY_RUN -eq 1 ]]; then
    echo "[1b] sweep last   (index=${POINT_LAST_INDEX}, singleton, time=${GPU_TIME})  [DRY RUN]"
    ...
    SWEEP_LAST_JOB=$(_rysy_sbatch \
        ...
        --time="$GPU_TIME" \
        ${_RYSY_QOS_FLAG:+"$_RYSY_QOS_FLAG"} \
        --job-name="cc_sw_last_${RUN_NAME}" \
        --output="${LOGS_DIR}/sweep_last_%j.out" \
        --error="${LOGS_DIR}/sweep_last_%j.err" \
        "$CLUSTER_DIR/job_gpu.sh" \
            python -m create_characteristic.stages.gpu.run_point \
                --run-dir "$RUN_DIR" \
                --point-index "$POINT_LAST_INDEX")
    echo "[1b] sweep last   -> Rysy job $SWEEP_LAST_JOB  (index=${POINT_LAST_INDEX}, time=${GPU_TIME})"
    if [[ $WAIT_FOR_COMPLETION -eq 1 ]]; then
