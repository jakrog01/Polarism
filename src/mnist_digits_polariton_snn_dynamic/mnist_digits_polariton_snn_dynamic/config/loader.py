...
class GeometryConfig:
    ...
    type
        Geometry provider dispatch key, currently ``"grid_lattice"`` only.
    n_side
        Grid side length for ``type="grid_lattice"``; total spots =
        ``n_side * n_side``.
    """
...
class EncodingConfig:
    ...
    type
        Encoder dispatch key, currently ``"linear_pixel"`` only.
    n_side
        Input side length for ``type="linear_pixel"``; encoder expects
        ``(N, n_side, n_side)``.
    """

    power_min: float
    power_max: float
    type: str = "linear_pixel"
    n_side: int = 7
...
def build_geometry(cfg: GeometryConfig) -> GeometryProvider:
    """Build a geometry provider from typed configuration."""
    if cfg.type == "grid_lattice":
        return GridLatticeGeometry(
            n_side=int(cfg.n_side),
            pitch_um=float(cfg.pitch_um),
            sigma_space_um=float(cfg.sigma_space_um),
            center_x_um=float(cfg.center_x_um),
            center_y_um=float(cfg.center_y_um),
        )
    raise ValueError(f"Unsupported geometry.type: {cfg.type!r}")


def build_encoder(cfg: EncodingConfig) -> Encoder:
    """Build an input encoder from typed configuration."""
    if cfg.type == "linear_pixel":
        return LinearPixelEncoder(
            n_side=int(cfg.n_side),
            power_min=float(cfg.power_min),
            power_max=float(cfg.power_max),
        )
    raise ValueError(f"Unsupported encoding.type: {cfg.type!r}")
