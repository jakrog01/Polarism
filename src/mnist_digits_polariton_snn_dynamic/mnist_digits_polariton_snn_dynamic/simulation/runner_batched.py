...
from mnist_digits_polariton_snn_dynamic.simulation.runner import _output_steps, _pulse_time_factors

LOGGER = logging.getLogger(__name__)

try:
    from cupy.cuda.memory import OutOfMemoryError as _CuPyOOM
except Exception:  # numpy-only environments; sentinel that never matches
    class _CuPyOOM(Exception):
        pass


def _free_gpu_pool(xp) -> None:
...
def simulate_batch(
    ...
    if p.ndim != 2 or p.shape[1] != resources.n_spots:
        raise ValueError(f"powers_batch shape must be (N, {resources.n_spots}), got {p.shape}")
    if p.shape[0] == 0:
    ...
    while start < n_samples:
        stop = min(start + current_batch, n_samples)
        try:
            _simulate_chunk(
                resources,
                p[start:stop],
            ...
            LOGGER.warning(
                "OOM at batch=%d (samples [%d:%d]); halving to %d and retrying.",
                current_batch, start, stop, new_batch,
            )
