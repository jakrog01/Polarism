...
from mnist_digits_polariton_snn_dynamic.simulation.resources import PulseScalars, SharedSimResources
...
class SimulationTraces:
    ...
    ----------
    psi
        Condensate density traces with shape ``(T_out, n_spots + 1)``.
    n_active
        Active reservoir traces with shape ``(T_out, n_spots + 1)``, or ``None``.
    n_inactive
...
def simulate_one_image(
    ...
    powers
        Float64 vector with shape ``(n_spots,)``.
    warmup_ps
    ...
    if p.shape != (resources.n_spots,):
        raise ValueError(f"powers shape must be ({resources.n_spots},), got {p.shape}")
    if not np.all(np.isfinite(p)):
        raise ValueError("powers must be finite")
    if getattr(cfg.physics, "init_seed", None) is None:
        raise ValueError("simulate_one_image requires physics.init_seed for deterministic init")
