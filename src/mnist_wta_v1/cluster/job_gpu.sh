...
if [[ "${POLARITON_ARRAY_DUMMY_MAX:-0}" == "1" \
    ...
    echo "Dummy Slurm array placeholder task ${SLURM_ARRAY_TASK_ID}; exiting."
    exit 0
fi
...
MNIST_WTA_DIR="$PROJECT_ROOT/src/mnist_wta_v1"
...
if [[ -f .venv/bin/activate ]]; then
    ...
    echo "ERROR: virtualenv not found in $PROJECT_ROOT" >&2; exit 1
fi

export PYTHONPATH="${PROJECT_ROOT}:${MNIST_WTA_DIR}:${MNIST_COMMON_DIR}:${PYTHONPATH:-}"
