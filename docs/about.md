# About

This project was created as part of the NCN OPUS grant, in collaboration with the [Polariton Exciton Research Group](https://polariton.fuw.edu.pl/).

<div class="about-grid">
  <div class="about-card">
    <img src="../assets/about/jakub-rogala.png" alt="Jakub Rogala">
    <div>
      <div class="about-role">Author</div>
      <div class="about-name">Jakub Rogala</div>
    </div>
  </div>
  <div class="about-card">
    <img src="../assets/about/krzysztof-tyszka.png" alt="Krzysztof Tyszka">
    <div>
      <div class="about-role">Scientific support</div>
      <div class="about-name">Krzysztof Tyszka, PhD</div>
    </div>
  </div>
  <div class="about-card">
    <img src="../assets/about/andrzej-opala.png" alt="Andrzej Opala">
    <div>
      <div class="about-role">Scientific support</div>
      <div class="about-name">Andrzej Opala, PhD</div>
    </div>
  </div>
</div>

## Partners
<div class="about-logo-stack">
  <div class="about-logo-panel">
    <img class="about-logo-light" src="../assets/about/polariton-logo-light.png" alt="Polariton Exciton Research Group logo">
    <img class="about-logo-dark" src="../assets/about/polariton-logo-dark.png" alt="Polariton Exciton Research Group logo">
  </div>
  <div class="about-logo-panel">
    <img class="about-logo-light" src="../assets/about/ncn-logo-light.png" alt="NCN logo">
    <img class="about-logo-dark" src="../assets/about/ncn-logo-dark.png" alt="NCN logo">
  </div>
  <div class="about-logo-panel">
    <img class="about-logo-light" src="../assets/about/logo_ICM.png" alt="ICM logo">
    <img class="about-logo-dark" src="../assets/about/logo_ICM.png" alt="ICM logo">
  </div>
</div>
